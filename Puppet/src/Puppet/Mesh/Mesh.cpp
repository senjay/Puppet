...
#if MESH_DEBUG_LOG
#define PP_MESH_LOG(...) PP_CORE_TRACE(__VA_ARGS__)
#else
#define PP_MESH_LOG(...)
#endif
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			PP_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		PP_CORE_INFO("Loading mesh: {0}", filename.c_str());

		m_Importer = std::make_unique<Assimp::Importer>();

		const aiScene* scene = m_Importer->ReadFile(filename, s_MeshImportFlags);
		if (!scene || !scene->HasMeshes())
			PP_CORE_ERROR("Failed to load mesh file: {0}", filename);
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			PP_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
			PP_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
			...
			{
				PP_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
		...
		if (m_IsAnimated)
		{
			for (size_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				for (size_t i = 0; i < mesh->mNumBones; i++)
				{
					...
					{
						PP_MESH_LOG("Found existing bone in map");
		...
		{
			PP_MESH_LOG("---- Materials - {0} ----", filename);
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				PP_MESH_LOG("  {0} (Index = {1})", aiMaterialName.data, i);
				aiString aiTexPath;
				uint32_t textureCount = aiMaterial->GetTextureCount(aiTextureType_DIFFUSE);
				PP_MESH_LOG("    TextureCount = {0}", textureCount);
				...
				PP_MESH_LOG("    COLOR = {0}, {1}, {2}", aiColor.r, aiColor.g, aiColor.b);
				PP_MESH_LOG("    ROUGHNESS = {0}", roughness);
				...
				if (hasAlbedoMap)
				{
					...
					PP_MESH_LOG("    Albedo map path = {0}", texturePath);
					...
					{
						PP_CORE_ERROR("Could not load texture: {0}", texturePath);
				...
				if (aiMaterial->GetTexture(aiTextureType_NORMALS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
					PP_MESH_LOG("    Normal map path = {0}", texturePath);
				...
				if (aiMaterial->GetTexture(aiTextureType_SHININESS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
					PP_MESH_LOG("    Roughness map path = {0}", texturePath);
					auto texture = Texture2D::Create(texturePath);
					if (texture->IsLoaded())
					{
				...
				if (aiMaterial->Get("$raw.ReflectionFactor|file", aiPTI_String, 0, aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						PP_MESH_LOG("    Metalness map path = {0}", texturePath);
				...
				{
					PP_MESH_LOG("    No metalness texture");
				...
				for (uint32_t i = 0; i < aiMaterial->mNumProperties; i++)
				{
					...
					PP_MESH_LOG("Material Property:");
					PP_MESH_LOG("  Name = {0}", prop->mKey.data);
					// PP_MESH_LOG("  Type = {0}", prop->mType);
					// PP_MESH_LOG("  Size = {0}", prop->mDataLength);
					float data = *(float*)prop->mData;
					PP_MESH_LOG("  Value = {0}", data);

					switch (prop->mSemantic)
					{
					case aiTextureType_NONE:
						PP_MESH_LOG("  Semantic = aiTextureType_NONE");
						break;
					case aiTextureType_DIFFUSE:
						PP_MESH_LOG("  Semantic = aiTextureType_DIFFUSE");
						break;
					case aiTextureType_SPECULAR:
						PP_MESH_LOG("  Semantic = aiTextureType_SPECULAR");
						break;
					case aiTextureType_AMBIENT:
						PP_MESH_LOG("  Semantic = aiTextureType_AMBIENT");
						break;
					case aiTextureType_EMISSIVE:
						PP_MESH_LOG("  Semantic = aiTextureType_EMISSIVE");
						break;
					case aiTextureType_HEIGHT:
						PP_MESH_LOG("  Semantic = aiTextureType_HEIGHT");
						break;
					case aiTextureType_NORMALS:
						PP_MESH_LOG("  Semantic = aiTextureType_NORMALS");
						break;
					case aiTextureType_SHININESS:
						PP_MESH_LOG("  Semantic = aiTextureType_SHININESS");
						break;
					case aiTextureType_OPACITY:
						PP_MESH_LOG("  Semantic = aiTextureType_OPACITY");
						break;
					case aiTextureType_DISPLACEMENT:
						PP_MESH_LOG("  Semantic = aiTextureType_DISPLACEMENT");
						break;
					case aiTextureType_LIGHTMAP:
						PP_MESH_LOG("  Semantic = aiTextureType_LIGHTMAP");
						break;
					case aiTextureType_REFLECTION:
						PP_MESH_LOG("  Semantic = aiTextureType_REFLECTION");
						break;
					case aiTextureType_UNKNOWN:
						PP_MESH_LOG("  Semantic = aiTextureType_UNKNOWN");
						break;
					}
#endif
		...
		m_IndexBuffer = IndexBuffer::Create(m_Indices.data(), m_Indices.size() * sizeof(Index) /sizeof(uint32_t));
	...
	void Mesh::TraverseNodes(aiNode* node, const glm::mat4& parentTransform, uint32_t level)
	{
		...
		// PP_MESH_LOG("{0} {1}", LevelToSpaces(level), node->mName.C_Str());
	...
	{
		PP_CORE_ASSERT(pNodeAnim->mNumRotationKeys > 0);
	...
	{
		PP_CORE_ASSERT(pNodeAnim->mNumScalingKeys > 0);
	...
	glm::vec3 Mesh::InterpolateTranslation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		PP_CORE_ASSERT(NextPositionIndex < nodeAnim->mNumPositionKeys);
		float DeltaTime = (float)(nodeAnim->mPositionKeys[NextPositionIndex].mTime - nodeAnim->mPositionKeys[PositionIndex].mTime);
		float Factor = (animationTime - (float)nodeAnim->mPositionKeys[PositionIndex].mTime) / DeltaTime;
		PP_CORE_ASSERT(Factor <= 1.0f, "Factor must be below 1.0f");
	...
	glm::quat Mesh::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		PP_CORE_ASSERT(NextRotationIndex < nodeAnim->mNumRotationKeys);
	...
	glm::vec3 Mesh::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		PP_CORE_ASSERT(nextIndex < nodeAnim->mNumScalingKeys);
	...
	void Mesh::DumpVertexBuffer()
	{
		...
		PP_MESH_LOG("------------------------------------------------------");
		PP_MESH_LOG("Vertex Buffer Dump");
		PP_MESH_LOG("Mesh: {0}", m_FilePath);
		if (m_IsAnimated)
		{
			for (size_t i = 0; i < m_AnimatedVertices.size(); i++)
			{
				auto& vertex = m_AnimatedVertices[i];
				PP_MESH_LOG("Vertex: {0}", i);
				PP_MESH_LOG("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
				PP_MESH_LOG("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
				PP_MESH_LOG("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
				PP_MESH_LOG("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
				PP_MESH_LOG("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
				PP_MESH_LOG("--");
			}
		}
		else
