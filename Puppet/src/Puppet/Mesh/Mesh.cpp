...
namespace Puppet {
	...
	void Mesh::Draw(const glm::mat4& transform, const glm::vec3& cameraPos, int entityID)
	{
		for (unsigned int i = 0; i < mSubMeshes.size(); ++i)
			mSubMeshes[i].Draw(transform, cameraPos, m_Material[0]->GetShader(), entityID, this);
	}
	...
	void Mesh::LoadModel(const std::string& path)
	{
		...
		std::filesystem::path standardFullPath = path;
		const aiScene* scene = importer.ReadFile(standardFullPath.string(), aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace);

		if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode)
		{
			PP_CORE_ERROR("ERROR::ASSIMP::{0}", importer.GetErrorString());
			return;
		}

		mDirectory = standardFullPath.parent_path().string();
		uint32_t subMeshIndex = 0;
		if (scene->HasAnimations())
		{
			bAnimated = true;
			ProcessNode(scene->mRootNode, scene, subMeshIndex);
			m_Animation = Animation(standardFullPath.string(), this);
		...
		if (bAnimated)
		{
			for (size_t boneIndex = 0; boneIndex < mesh->mNumBones; ++boneIndex)
			{
				...
				for (int weightIndex = 0; weightIndex < numWeights; ++weightIndex)
				{
					...
					PP_CORE_ASSERT(vertexId <= vertices.size());
					for (int i = 0; i < MAX_BONE_INFLUENCE; ++i)
					{
	...
	std::optional<std::vector<MaterialTexture>> Mesh::loadMaterialTextures(aiMaterial* mat, aiTextureType type, uint32_t& subMeshIndex)
	{
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			bool skip = false;
			//if (!m_Material[subMeshIndex]->mTextures.empty())
			//{
			for (unsigned int j = 0; j < m_Material[subMeshIndex]->mTextures.size(); j++)
			{
				if (std::strcmp(m_Material[subMeshIndex]->mTextures[j].path.data(), str.C_Str()) == 0)
				{
					textures.push_back(m_Material[subMeshIndex]->mTextures[j]);
			...
			{   // if texture hasn't been loaded already, load it
				...
				{
					PP_CORE_WARN("Load Texture failed!");
					texture.texture2d = Texture2D::Create("./assets/textures/DefaultTexture.png");
				}
				...
				case aiTextureType_BASE_COLOR:
					...
					m_Material[subMeshIndex]->mAlbedoMap = texture.texture2d;
				...
				m_Material[subMeshIndex]->mTextures.push_back(texture);  // store it as texture loaded for entire model, to ensure we won't unnecesery load duplicate textures.
			}
		}
