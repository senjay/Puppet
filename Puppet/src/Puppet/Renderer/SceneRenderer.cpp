...
namespace Puppet {
	...
	std::pair<Ref<TextureCube>, Ref<TextureCube>> SceneRenderer::CreateEnvironmentMap(const std::string& filepath)
	{
		...
		if (!equirectangularConversionShader)
			equirectangularConversionShader = ShaderLibrary::GetInstance().Get("EquirectangularToCubeMap");
		Ref<Texture2D> envEquirect = Texture2D::Create(filepath);
		PP_CORE_ASSERT(envEquirect->GetFormat() == TextureFormat::Float16, "Texture is not HDR!");
		...
		if (!envFilteringShader)
			envFilteringShader = ShaderLibrary::GetInstance().Get("EnvironmentMipFilter");
		...
		if (!envIrradianceShader)
			envIrradianceShader = ShaderLibrary::GetInstance().Get("EnvironmentIrradiance");
